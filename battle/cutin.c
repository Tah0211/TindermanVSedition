...
static void fade_out(SDL_Renderer* r, int w, int h, int ms)
{
    ...
    while (elapsed < ms) {
        ...
        uint8_t a =clamp_u8 (uint8_t)(t * 255.0f);
        draw_fade(r, w, h, a);
        SDL_RenderPresent(r);
...
// // ======================================================
// // mpv 検出
// //  - 学内PC(glibc 2.31)では ./tools/mpv/mpv が起動不能の可能性が高い
// //  - そこで ./tools/mpv/mpv.AppImage を最優先にする
// // ======================================================
...
// {
//     // AppImage を最優先
//     if (has_bundled_mpv_appimage()) return true;
//     if (has_bundled_mpv_bin()) return true;
//     return false;
// }
...
//     if (pid == 0) {
//         // 同梱優先順位：AppImage → 同梱bin → system mpv
...
//         // AppImage は内部に依存を抱えているので LD_LIBRARY_PATH は触らない。
//         // 同梱bin を使う場合のみ、同梱libを優先（既存を壊さず先頭に追加）
//         if (use_bin) {
//             prepend_ld_library_path("./tools/mpv/lib");
//         }
...
//         // 2P側は左右反転（--vf=hflip）
//         //
//         // 学内PCでは GPU 周りでコケる可能性があるため、
//         // AppImage の場合は --no-gpu --vo=x11 を足して安定寄りにする。
//         //
//         // ※ AppImage 単体テストで問題なければ、このままでも動きます。
//         //    もし他PCで画質/性能を優先したければ、ここは後で調整可能です。

//         if (!flip_h) {
//             char* const argv_appimage[] = {
//                 (char*)exec_path,
//                 (char*)"--fullscreen",
//                 (char*)"--ontop",
//                 (char*)"--no-border",
...
//             char* const* argv = use_appimage ? argv_appimage : argv_normal;

//             if (use_bundled) {
//                 execv(exec_path, (char* const*)argv);
//             } else {
//                 execvp(exec_path, (char* const*)argv);
//             }
//         } else {
//             char* const argv_appimage_flip[] = {
//                 (char*)exec_path,
//                 (char*)"--fullscreen",
//                 (char*)"--ontop",
//                 (char*)"--no-border",
...
//             char* const* argv = use_appimage ? argv_appimage_flip : argv_flip;
